...
namespace ITParkMongoDB
{
    class DataBaseMethods
    {
        ...
        public static List<Product> Shopping(string shopping, Product newCount)
        {
            var client = new MongoClient("mongodb://localhost");
            var database = client.GetDatabase("Magnit");
            var collection = database.GetCollection<Product>("Bakery");
            collection.ReplaceOne(x => x.NameOfProduct == shopping, newCount);
            return collection.Find(x => true).ToList();

            //if (list.Count != 0)
            //{
            //    System.Console.Write("Shoping list:");
            //    foreach (var item in list)
            //    {
            //        System.Console.WriteLine($"{item.NameOfProduct} {item.Price}{item.CountAtWarehouse}");
            //    }
            //    newCount.CountAtWarehouse -= newCount.CountAtWarehouse;
            //}
            //else
            //    System.Console.WriteLine("Current product not exist in data base");
        }
